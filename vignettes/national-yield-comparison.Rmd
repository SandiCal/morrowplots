...
This exercise demonstrates how to use ggplot2 to create a graph comparing corn yields on the Morrow Plots to the U.S. national average. We'll use the data from the morrowplots R package, and we'll also download and import national average data from the National Agricultural Statistics Service (NASS).
...
NASS will give your file a super long name, so rename it "NASS_data.csv" instead. Then you need to make sure R knows where to find it, which you can do in one of two ways:

1) You can move the NASS file to your working directory. In your R console, use getwd() to identify your working directory and then manually move the file to that folder. 
...
## only run this code if you chose Option 1: Download NASS data yourself
## delete the ## before NASS below to activate the code
## if your NASS file is not in your working directory, insert the file path before the file name
