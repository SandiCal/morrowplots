...
You will need to install the morrowplots and tidyverse packages if you haven't already.
...
mp3 <- 
  ...
  filter(morrowplots, plot_num == 3, !is.na(yield_bush))  %>% 
  ...
  summarise(mp_ave = mean(yield_bush))
...
ggplot(data = mp3) +
  geom_line(aes(x= year, y = mp_ave, color = treated))
```
...
ggplot(data = mp3) +
  geom_smooth(aes(x= year, y = mp_ave, color = treated))
```
...
NASS <- select(NASS, year = Year, nat_ave = Value)

mp3_NASS <-
  mp3 %>% 
  left_join(NASS, by = "year")
...
ggplot(data = mp3_NASS) +
  
  ## mp_ave color coded by 'treated'
  geom_smooth(aes(x= year, y = mp_ave, color = treated))+
  
  ## add nat_ave as a dashed black line in the same graph
  geom_smooth(aes(x= year, y = nat_ave), linetype = "dashed", color = "black")+
  
  ## add title and subtitle
  labs(title = "Morrow Plots Continuous Corn Yield Trends in Bushels per Acre",
...
You did it! You combined datasets and graphs, plus you bothered to read through the interpretation, which shows you really care about data. Well done! You deserve a break and a treat.
